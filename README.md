...
部署要点（均为线上实证，踩坑记录见 `RAG.md` 与提交历史）：
...
- **定时任务**：看门狗（每 5 分钟自愈）+ 夜间 02:30 销量导入 → 条件触发稠密重建（`RAG.md` §7）
- **备案**：中国内地节点必须完成 ICP 备案；页脚备案号由环境变量 `ICP_NUMBER` 注入并链接工信部
...
- 仓库自带密钥扫描器（`reviewer/scan_secrets.py`），提交前扫描；`.gitignore` 覆盖 `*.pem` / `*.key` 等

**访问控制**
- 管理后台独立 Bearer 凭据（≥32 位 CSPRNG，由运维注入），不开放注册；凭据缺失返回 503、无效返回 401，
...
**容器与网络**
- 容器端口只绑 `127.0.0.1`，外部统一经宿主 nginx；Redis 不发布端口

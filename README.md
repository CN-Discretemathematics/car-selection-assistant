...
| pair-coverage@5（对比两侧证据在场） | 0.4390 | 0.4390 |

semantic 桶旧口径 Hit@5 0.1463 → v2 **1.0**：旧数字主要是口径失真（一题多解被记为不相关）；
新口径暴露的真实短板——compare 两侧覆盖 0.439、参数事实覆盖 0.766、recommend 约束有效率
0.477——才是下一轮优化的靶子。评测规范详见 [RAG.md](RAG.md) §4.5。
...
机制：对比类查询按锚定车系**交错双侧证据**（缺席侧自动补召回），未点名车系的
推荐/语义查询按解析出的硬约束**约束优先重排**——点名车系的问答不受影响。

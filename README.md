...
需求对齐判定后，被掩盖的真实能力与短板同时现形。**下表是最新一次测评（v4 检索侧闭环 +
compare 回退，2026-09-11）的真值**：

| v2 指标（451 题） | sparse 基线 | pipeline（生产） |
| --- | --- | --- |
| fact-coverage@5（参数事实出现在证据中） | 0.7234 | **0.7660** |
| valid-hit@5（推荐/语义，约束满足即相关） | 0.4167 | **0.8136** |
...
覆盖 0.766、recommend 约束有效率 0.5517 → 0.6992（约束下推 + 解析语义修复）。
评测规范与**历史口径归档表**详见 [RAG.md](RAG.md) §4.2。
...
```powershell
cd backend
python -m pytest -q                   # 548 用例（随迭代增长，以实际输出为准）
cd ..\web
...
RAG.md            # RAG 子系统设计与运维文档（选型依据与逐轮实测见 RAG.md 自身）
```

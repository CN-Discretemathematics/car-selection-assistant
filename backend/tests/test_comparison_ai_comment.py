...
def test_ai_comment_returns_grounded_text(monkeypatch):
    ...
    # prompt 必须显式禁数字与营销话术
    assert "不得出现任何数字" in fake.__dict__.get("_text", "") or True  # prompt 内容见下方断言
    messages = build_comment_messages(_analysis())
    assert "不得出现任何数字" in messages[0]["content"]
    assert "已核实事实" in messages[1]["content"]


def test_ai_comment_rejects_digits(monkeypatch):
    """LLM 复述了任何数字 = 失控（数字归确定性层），点评必须丢弃。"""
    fake = _FakeClient("甲车动力高 50kW，建议选甲。")
    monkeypatch.setattr(ai_summary, "get_llm_client", lambda: fake)

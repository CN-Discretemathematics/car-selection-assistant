...
def validate_tool_answer(
    ...
    语义边界（2026-09-17 评审二轮 §2，如实声明）：本契约防的是「凭空数字/凭空名字」，
    **不校验数字/名字与所述对象的归属**——已知放行面：/10000 万元派生的跨实体复用、
    名字内数字入池、双向子串近似（「奔驰E」≈「奔驰EQE」）、纯 CJK 车名不做形状候选
    （宁漏勿错，漏判由数字契约兜底）。调用方应把用户消息与画像数值先并入
    allowed_numbers，否则复述用户预算/人数/年份会被误杀（评审二轮实测）。
    """
    violations: list[str] = []

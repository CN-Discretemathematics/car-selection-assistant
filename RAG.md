...
#### v3 落地（2026-09-11）：不可回答题 / 多约束推荐 / 口语化改写

三类扩充全部由生成器自动产出（独立随机源追加，原 520 题逐字不变、可复现）：

1. **不可回答题（60 题，单独成库 `eval/questions-unanswerable.json`）**：锚定车系在某
   参数探针维度上完全无数据（与线上 probe_facts 同口径——问 WLTC 续航但车上有 CLTC
   续航算可答），期望「官方资料未披露」；
2. **多约束推荐题（80 题）**：预算/能源/车身/座位 2~4 个硬约束组合，满足车系集合由
   `series_satisfies` 计算并随题输出 satisfying_count（限定 2~80 保证区分度）；
3. **口语化改写变体（167/600 题改写成功）**：DeepSeek 改写 + 硬校验（数字逐组一致、
   车系名不变），单独成库 `eval/questions-paraphrased.json`。

**复测结果与两个新发现**：

- **拒答诚实性：修复前 0/60 → 修复后 60/60（100%）**。不可回答题暴露了一个真实产品
  bug：问到的参数维度 DB 完全没有时，`build_series_qa_answer` 沉默跳过、只回车系画像，
  不标注「官方资料未披露」。修复：`missing_param_labels` 按探针维度检测缺失并显式标注
  （`tests/test_series_qa.py` 回归用例覆盖）；
- **多约束推荐题的口径对照**：旧口径 Hit@5 **0.075**（锚定车系只是几十个合法答案之一）
  vs v2 valid-hit@5 **0.65**、valid-precision@5 0.445——同题 8.7 倍差，坐实旧口径对
  一题多解查询的失真；
- **口语化改写的鲁棒性损失（词面泄漏量化）**：分桶匹配对比——参数题 fact-coverage
  0.766 → **0.667**（−10pt）；对比题 pair-coverage 0.439 → **0.353**（−9pt）。用户换一种
  说法，检索质量可测量地下降；改写体总体 Hit@5 0.9722 偏高是构成偏差（数字硬校验使
  高命中桶过采样），分桶对比才是有效读法；改写成功率 167/600（28%）：数字逐组保留 +
  车系名不变的硬校验拒绝了大部分改写（宁可错杀，防止把坏变体算成检索失败）。

...
1. **analyze（查询理解）**：用 `app/catalog/series_index.py` 的名称索引（车系名/品牌+车系名/别名，归一化子串匹配、最长优先去重）解析消息中的真实车系；生成实体增强查询（别名 → 规范名）与元数据过滤（单车系自动加 `series_id`）。确定性实现、不依赖 LLM。空查询短路到 END。
...
| pair-coverage@5（compare，79 题） | 0.439 | 0.4634 |
...
- **compare 实体解析错位**：部分对比题的实体解析结果与锚定车系不一致（款型名
  「sDrive25Li X设计套装」解析为宝马 iX 而非 X1），强行插入错误车系会挤掉正确侧
  ——双侧覆盖优化三版实现均未提升 pair-coverage，已回退（机制与单测保留）。
  修正解析错位后重启。
- **31 车系索引覆盖缺口**：全部款型停售的车系不在索引内，相关问答降级展示归档数据。
- **改写成功率**：26%——剩余失败多为改写时丢失车系名/约束，需 prompt 与校验继续放宽。
- **语义桶扩样**：semantic 变体仅 4 条，HyDE 与相关性阈值待扩样后复验。

...
- ~~compare 实体解析错位~~（**v6.1 已修复**）：名称索引扩展在售款型显示名→车系
  （对比题以款型名表述，此前解析器只能词面误配——解析准确率 34%（27/79），
  修复后 **88.6%（70/79）、完全错配 0**）。配合 compare 双侧召回保障，
  **pair-coverage@5 0.439 → 0.7073（+61%）**，compare hit@5 保持 0.9873，
  NDCG@10 0.5447 创新高，其余指标零回归。剩余 9 题 partial（款型名部分命中）
  与 31 车系覆盖缺口同源；
